...
if (size(img1) ~= size(img2))
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 1')
   return;
end
...
if ((M < 11) | (N < 11))
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 2')
   return
end

if (length(K) ~= 2)
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 3')
   return;
end

if (K(1) < 0 | K(2) < 0)
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 4')
   return;
end
...
if ((H*W)<4 | (H>M) | (W>N))
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 5')
   return;
end
...
if (min_img_width < max_win_width)
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 6')
   return;
end

if (length(weight) ~= level | sum(weight) == 0)
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 7')
   return;
end

if (method ~= 'wtd_sum' & method ~= 'product')
   overall_mssim = -Inf;
   display('ssim_mscale_new: CHECKPOINT 8')
   return;
end
...
%   overall_mssim = prod(mssim_array.^weight);
   ...
   %%%%%%%%%%%%%%%%%%% MY NORMALIZATION %%%%%%%%%%%%%%%%%%%%%%%%
   overall_mssim = overall_mssim/(1 + sum(weight(1:level-1)));
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
